...
/// 在 SettingGroup 上设置 <see cref="HeaderWidth"/> 等价于在容器上设置 SettingItem.HeaderWidth 附加属性，
...
public class SettingGroup : HeaderedItemsControl
{
    ...
    #region HeaderWidth (转发到 SettingItem.HeaderWidth 附加属性)

    /// <summary>
    /// 获取或设置组内所有 SettingItem 的标题列宽度。
    /// 本属性直接转发 <see cref="SettingItem.HeaderWidthProperty"/> 附加属性的存储，
    /// 不引入新的依赖属性，避免重复定义。
    /// </summary>
    public GridLength HeaderWidth
    {
        get => SettingItem.GetHeaderWidth(this);
        set => SettingItem.SetHeaderWidth(this, value);
    }

    #endregion HeaderWidth
